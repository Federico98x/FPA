...
  }, [soundscapes]);
  ...
  // ... (handleSelectSoundscape, handlePlayPause remain the same)
  const handleSelectSoundscape = (soundscape: Soundscape) => {
    setCurrentSoundscape(soundscape);
    // Optional: auto-play when a new soundscape is selected
    if (!isPlaying) {
      setIsPlaying(true);
    }
  };
...
  const handleGenerationComplete = (newSoundscape: Soundscape) => {
    // Add the AI generation to the main list so it appears in the UI temporarily
    setSoundscapes(prev => {
...
  const handleSaveMix = (soundscapeToSave: Soundscape) => {
    ...
    // Update the existing entry in the list (whether curated or AI generated)
