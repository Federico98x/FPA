...
import { useAudioEngine } from '@/hooks/use-audio-engine'; // FIX: Import the audio engine hook
...
export default function SoundscapePlayer({ currentSoundscape, isPlaying, onPlayPause, onSaveMix }: SoundscapePlayerProps) {
  // FIX: Initialize the audio engine
  useAudioEngine(currentSoundscape, isPlaying);

  // Logic for save button visibility: Can save if it's not already custom
  const canBeSaved = currentSoundscape && !currentSoundscape.isCustom;
  const isSaved = currentSoundscape && currentSoundscape.isCustom;
...
              <CardDescription className="mt-1">{currentSoundscape.description}</CardDescription>
              {/* ENHANCEMENT: Display the mix elements */}
              <div className="mt-2 text-sm text-muted-foreground italic">
                {currentSoundscape.mix.join(' + ')}
              </div>
            </div>
...
              )}
               {isSaved && (
                <Button variant="outline" size="icon" className="rounded-full" disabled aria-label="Mix saved">
